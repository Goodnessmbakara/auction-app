...
import fs from "fs";
import { createHelia } from '@helia/http';
import { unixfs } from '@helia/unixfs';
import { Readable } from "stream";

export const config = { api: { bodyParser: false } };

// Initialize Helia with HTTP client
...
async function uploadToIPFS(file: formidable.File): Promise<string> {
  ...
  try {
    ...
    const cid = await fs.add(fileBuffer);
    console.log("[CreateAuctionAPI] IPFS upload successful. CID:", cid.toString());
